...
function calculateRisks(entries: LifestyleEntry[]): HealthRisk[] {
  if (entries.length < 3) {
    return [];
  }
  ...
  if (avgActivity < 30) { cardioRisk += 20; cardioFactors.push("Low physical activity"); }
  ...
  if (avgActivity < 20) { diabetesRisk += 25; diabetesFactors.push("Sedentary lifestyle"); }
  ...
  if (avgActivity < 30) { obesityRisk += 20; obesityFactors.push("Insufficient exercise"); }
...
export function RiskPredictions({ entries }: RiskPredictionsProps) {
  const risks = useMemo(() => calculateRisks(entries), [entries]);

  return (
    <div className="p-6 rounded-xl bg-card border border-border">
      <div className="mb-6">
        <h3 className="text-lg font-display font-semibold text-foreground">Health Risk Predictions</h3>
        <p className="text-sm text-muted-foreground">Pattern-based risk analysis (see Medical Risk Scores for validated calculators)</p>
      </div>
